"""
Pictology pipeline test — Rembrandt authenticated works.

Runs each analysis function individually on all Rembrandt authenticated images
and writes per-function and per-image results to a set of CSV files under
outputs/pictology_rembrandt/.
"""
...
# ── Configuration ──
DATA_DIR = ROOT / "data" / "raw" / "rembrandt" / "authenticated"
OUT_DIR  = ROOT / "outputs" / "pictology_rembrandt"
...
print(f"Found {len(image_paths)} Rembrandt authenticated images in {DATA_DIR}")
    ...
    for func_name, prefix, fn in ANALYSES:
        ...
        for k, v in raw.items():
            if isinstance(v, (int, float, bool, np.integer, np.floating, np.bool_)):
                ...
                # Flatten short lists into indexed columns
